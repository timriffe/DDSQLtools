...
* dataProcessTypeIds = `get_dataprocess()`
...
To explore all possicle indicatorIds, indicatorTypeIds and components, `get_iitypes` can be of use. Although `get_indicators` and `get_indicatortypes` are very helpful for looking at the entire list of indicators, a user often wants to combine the two. That's where `get_iitypes` comes in. For example:
...
The output is all IndicatorTypeID, together with the IndicatorIds and ComponentIDs. The advantage of `get_iitypes` is that you can select any of the with the function:
